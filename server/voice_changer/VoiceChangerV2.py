...
class VoiceChangerV2:
    ...
    def process_audio(self, audio_in: AudioInOutFloat) -> tuple[AudioInOutFloat, float]:
        ...
        if audio is None:
            # In case of silence, still render background tracks
            mixed_t = torch.zeros(block_size, dtype=torch.float32, device=self.device_manager.device)
            if self.audio_mixer is not None:
                try:
                    ...
                    if isinstance(bg, torch.Tensor):
                        mixed_t = mixed_t + bg.to(mixed_t.device, dtype=mixed_t.dtype)
                except Exception:
                    # Ignore mixer errors during silence
                    pass
